...
%             stimtemp = abs(trial_stim_run(n));
%             interval = sign(trial_stim_run(n));
%             if interval < 0
%                 interval = 2;
%             end

            %[response] = hardwarefun_Tactile(trial_stim_run(n),app);
            hardwarefun_Tactile(trial_stim_run(n),app);
%             [response_temp] = tactile_hardware_testing(stimtemp, interval, filename, subject_number, k);
%             
%             if response_temp == 1
%                response = sign(trial_stim_run(n));
%             else
%                 response = -1*sign(trial_stim_run(n));
%             end
                %if response is 1 then it's correct
...
if fit_on
    ...
    mu_est = -b(1)/b(2);
    sigma_est = 1/b(2);
    display(['Threshold = ', num2str(sigma_est), 'or', num2str(sigma_est*0.2*2), '']);
end

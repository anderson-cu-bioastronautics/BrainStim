...
%         display(['Trial ', num2str(n), ' Stim = ', num2str(trial_stim_run(n))])
        ...
        %not complete
            ...
            display(['NOTICE ME SENPAI', trial_stim_run(n)])
            [response] = runscreen(trial_stim_run(n),screens,screenNumber,white,grey,black,inc,window,windowRect,app);
...
if fit_on
    ...
    mu_est = -b(1)/b(2);
    sigma_est = 1/b(2);
    display(['Threshold = ', num2str(sigma_est), ' and mu is ', num2str(mu_est), ''])
end

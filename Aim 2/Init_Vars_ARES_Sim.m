...
if trial_num <= 12
    ...
    if trial_num <= 8
        m_fuel0 = 60; % High fuel training
    else
        m_fuel0 = 50; % Low fuel training
    end
else
...
fail_type=4; % 1=Pitch; 2=roll;3=cont rev;4=stuck thruster;5=fuel gauge 6=roll triag 
fail_time=1300;
fail_detect=0;
...
quad_s = 1;
%% Map Selection
% Loads a map and its files

if scenario_num <= 4
    nhname = sprintf('HSD_Maps\\map_0%d_nh.png',scenario_num); %Image no hazards
    yhname = sprintf('HSD_Maps\\map_0%d_nh.png',scenario_num); %Image with hazards (hidded for training)
    dtname = sprintf('HSD_Maps\\map_0%d_data.mat',scenario_num); %Map Data
    hsname = sprintf('HSD_Maps\\blank_hsi_map.png'); %HSI map
elseif scenario_num < 10
    nhname = sprintf('HSD_Maps\\map_0%d_nh.png',scenario_num); %Image no hazards
    yhname = sprintf('HSD_Maps\\map_0%d_yh.png',scenario_num); %Image with hazards
    dtname = sprintf('HSD_Maps\\map_0%d_data.mat',scenario_num); %Map Data
    hsname = sprintf('HSD_Maps\\map_0%d_haz.png',scenario_num); %HSI map
else
    nhname = sprintf('HSD_Maps\\map_%d_nh.png',scenario_num); %Image no hazards
    yhname = sprintf('HSD_Maps\\map_%d_yh.png',scenario_num); %Image with hazards
    dtname = sprintf('HSD_Maps\\map_%d_data.mat',scenario_num); %Map Data
    hsname = sprintf('HSD_Maps\\map_%d_haz.png',scenario_num); %HSI map
end

image_nh = imread(nhname);
image_yh = imread(yhname);
image_hs = imread(hsname);
load(dtname)
...
Initial_conditions = [Time3, m_fuel0, x0, y0, VE0, VN0,...
                      ...
                      fail_time,fail_time_s,fail_detect,quad_s];  
...
%% Tactile Stuff
set_ExEx; % sets  global variables in extrinsic matlab 

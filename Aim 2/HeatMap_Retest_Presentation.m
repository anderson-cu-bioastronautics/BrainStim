...
for i = 1:length(tasks)
    ...
    plot(xtickmarks,mean(TaskData(1:8,:))-mean(Taskmean(1:8,:)),[ '-k'],'LineWidth',1.25)
    for sub = 9:16
        PerformData = cell2mat(AllRetest_mat(sub));
        meansData = cell2mat(means_mat(sub))';
        TaskData(sub,:) = PerformData(i+1,:);
    ...
    plot(xtickmarks,mean(TaskData(9:16,:))-mean(Taskmean(9:16,:)),[ '-r'],'LineWidth',1.25)
    yline(0,'LineWidth',.4)
    hold off
    set(gca,'FontSize',13)
    TaskData = [];
...
for i = 1:8
    ...
    for sub = 1:16
        PerformData = cell2mat(AllRetest_mat(sub));
        meansData = cell2mat(means_mat(sub))';
        stdData = cell2mat(std_mat(sub))';
        TaskData(sub,:) = PerformData(i+1,:);
        Taskmean(sub,:) = meansData(i+1,:);
        Taskstd(sub,:) = stdData(i+1,:);
        %         plot(xtickmarks,TaskData(sub,:)-Taskmean(sub),['-*' colors(sub)],'LineWidth',1.15)
   %     plot(xtickmarks,TaskData(sub,:)-Taskmean(sub),[ symbs(sub) 'k'],'LineWidth',1.15, 'MarkerFaceColor', 'k')
        %         plot(xtickmarks,TaskData(sub,:)-Taskmean(sub),'-*','LineWidth',1.15, 'Color', colors(sub, :), 'MarkerFaceColor', colors(sub, :))
        hold on
    ...
    plot(xtickmarks,mean(TaskData(1:16,:))-mean(Taskmean(1:16,:)),[ '-k'],'LineWidth',1.25)
    errorbar(xtickmarks,mean(TaskData(1:16,:))-mean(Taskmean(1:16,:)),std(TaskData(1:16,:))-std(Taskmean(1:16,:)),[ '-k'],'LineWidth',1.25)
    ylim([-.2 .2])
    yline(0,'LineWidth',.4)
    hold off
    set(gca,'FontSize',13)

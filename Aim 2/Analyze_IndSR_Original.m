...
x4mult = 1; row_ind = 1;
for trial = 1:length(SRlevels(1:6))*3
    % load in file data
    if trial >9
    ...
    if x4mult < 10
        ...
        file_str = ['Subject_' Subject_str '_Set' num2str(x4mult) '_T0' num2str(row_ind) '_RAW.mat'];
    end
    ...
    audStop_mat(row_ind+3,x4mult)=audStop(2);
    tacStop_mat(row_ind,x4mult)=tacStop(1);
    tacStop_mat(row_ind+3,x4mult)=tacStop(2);
    ...
    if floor(trial/(3*x4mult)) == 1 % help divide into SR columns
        x4mult = x4mult+1;
        row_ind = 1;
    else
    end
...
rms_total_p = zeros(1,length(SRlevels));
fracTimeIn_p = zeros(1,length(SRlevels));
Choice_P_p = zeros(1,length(SRlevels));
CAL_Score_p = zeros(1,length(SRlevels));
Head_Change_p = zeros(1,length(SRlevels));
audStop_p = zeros(1,length(SRlevels));
tacStop_p = zeros(1,length(SRlevels));

for i=1:length(SRlevels)
    
    for j=1:3
        DenomSize = numel(rms_ranked);
        rms_total_p(i) = rms_total_p(i)+rms_ranked(j,i)/(DenomSize*3);
        fracTimeIn_p(i) = fracTimeIn_p(i)+TimeIn_ranked(j,i)/(DenomSize*3);
        Choice_P_p(i) = Choice_P_p(i)+Choice_P_ranked(j,i)/(DenomSize*3);
        CAL_Score_p(i) = CAL_Score_p(i)+CAL_ranked(j,i)/(DenomSize*3);
        Head_Change_p(i) = Head_Change_p(i)+Head_Change_ranked(j,i)/(DenomSize*3);
    end
    
    for j=1:6
        DenomSize = numel(audStop_ranked);
        audStop_p(i) = audStop_p(i)+audStop_ranked(j,i)/(DenomSize*6);
        tacStop_p(i) = tacStop_p(i)+tacStop_ranked(j,i)/(DenomSize*6);
    end
...
if any(Retest)==0
    ...
    xlStart = 'G1'; %first column of data to write to assuming 7 original tests
    xlswrite(OrderFile, retestForFile, 1, xlStart)

    
    % save all this data
    SRPerfData = {SRlevels, rms_total_p, fracTimeIn_p, Choice_P_p, CAL_Score_p, Head_Change_p, audStop_p, tacStop_p, Total_Performance};
    savefilename = ['PerformanceData' Subject_str 'Orig.mat'];
    save(savefilename,'SRPerfData');
    
    SRRankData = {SRlevels, rms_ranked, TimeIn_ranked, Choice_P_ranked, CAL_ranked, Head_Change_ranked, audStop_ranked, tacStop_ranked};
    savefilename = ['RankedData' Subject_str 'Orig.mat'];

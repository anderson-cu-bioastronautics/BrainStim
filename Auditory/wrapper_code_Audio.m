...
cha = chami();
cha.open();
cha.abort_exams(); % In case any were inadvertently left in the queue

% Set-up Tone Generation

tg_exam = tone_generation_exam();
% Use exam's defaults, as those are what is used by CHA application


% Set Output Channel & level
...
%         display(['Trial ', num2str(n), ' Stim = ', num2str(trial_stim_run(n))])
        ...
        %not complete
            ...
            [response] = examplefunc_Audio(trial_stim_run(n),tg_exam);
...
%% Fit the data
if fit_on
    [b] = brglmfit(tstim_run, cor,'binomial','link','probit');
    mu_est = -b(1)/b(2);
    sigma_est = 1/b(2);
    display(['Threshold = ', num2str(sigma_est), ' dB or ', num2str(sigma_est*0.2*2), ' dB/s (if that can be said)'])
end
